...
#define JOFCLIENTVERSION JOF_VERSION_STRING " Sol's JoF" JOF_VERSION_SUFFIX

#define CLIENT_WINDOW_TITLE "Sol's JoF EJK (JoF " JOFVERSION ")"
#define CLIENT_CONSOLE_CORNER_DESCRIPTION "Sol's JoF EJK (JoF " JOFVERSION ") - " JOF_COMMIT_SHA_SHORT
#define CLIENT_CONSOLE_TITLE "Sol's JoF EJK Console"
...
typedef struct cvar_s {
	...
	int				hashIndex;
	qboolean		rendererOwned;		// registered through the renderer import table
	qboolean		rendererActive;		// available through the currently loaded renderer
} cvar_t;
...
#if !defined(DEDICATED) && !defined(_GAME) && !defined(UI)
typedef enum {
	VERSION_UNDEF,
	VERSION_1_00, //JKA
	VERSION_1_01, //JKA
	VERSION_1_01_CONSOLE, //JKA
	VERSION_1_02,
	VERSION_1_03,
	VERSION_1_04
} JKVersion_t;

typedef struct playerState16_s {
	int			commandTime;	// cmd->serverTime of last executed command
	int			pm_type;
	int			bobCycle;		// for view bobbing and footstep generation
	int			pm_flags;		// ducked, jump_held, etc
