...
void Com_ExecuteCfg(void)
{
	...
	if(!Com_SafeMode())
	{
		...
		Cbuf_Execute();
		Cbuf_ExecuteText(EXEC_NOW, "exec " Q3CONFIG_AUTO "\n");
		Cbuf_Execute();
	}
}
...
void Com_Init( char *commandLine ) {
	...
	//	Cvar_CheckRange( com_dedicated, 0, 2, qtrue );
		...
		Cmd_AddCommand("nvmlStatus", Sys_NVMLStatus_f, "Prints status of currently selected NVML device\n");
		Cmd_AddCommand("nvmlResetClocks", Sys_NVMLResetClocks_f, "");
		Cmd_AddCommand("nvmlAutoBoostedClock", Sys_NVMLAutoBoostedClock_f, "");
		Cmd_AddCommand("nvmlSetPowerLimit", Sys_NVMLSetPowerLimit_f, "");
#endif
#endif
...
//==================================================================
#if 1
static void Com_WriteConfigToFile( const char *filename, qboolean saveDefaults = qtrue ) {
	fileHandle_t	f;
	...
	if (saveDefaults) {
		...
		FS_Printf(f,	"// generated by" CLIENT_WINDOW_TITLE "\n"
						"//keybinds\n");
		Key_WriteBindings(f);
		FS_Printf(f,	"\n//cvars\n");
		Cvar_WriteModifiedVariables(f);
	}
	FS_FCloseFile( f );
}
#else
...
void Com_WriteConfig_f( void ) {
	...
	if (!Q_stricmp(Cmd_Argv(0), "write"))
		saveDefaults = qfalse;
	else if (Cmd_Argc() >= 3 && !Q_stricmpn(Cmd_Argv(2), "mod", 3))
		saveDefaults = qfalse;
	...
	if (saveDefaults && (!Q_stricmp(filename, PRODUCT_NAME ".cfg") || !Q_stricmp(filename, PRODUCT_NAME "global.cfg"))) {
		saveDefaults = qfalse;
	}
	...
	if (saveDefaults)
		...
		Com_Printf( "Writing user settings to %s\n", filename );

	Com_WriteConfigToFile(filename, saveDefaults);
}
#else
void Com_WriteConfig_f( void ) {
	char	filename[MAX_QPATH];
...
void Com_Frame( void )
{
	try
	{
		...
		int			timeAfter = 0;
		qboolean	busyWait = (qboolean)com_busyWait->integer;
