...
void SV_Init (void) {
	...
	sv_voice = Cvar_Get("sv_voice", "1", CVAR_ARCHIVE | CVAR_LATCH, "Enable server voice chat relay");
	Cvar_Get("sv_voiceProtocol", sv_voice->integer ? VOICE_PROTOCOL_NAME : "", CVAR_SYSTEMINFO | CVAR_ROM, "Server voice protocol");
	Cvar_Get("sv_voiceServer", "", CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_SYSTEMINFO, "External voice relay address (host:port)");
	Cvar_Get("sv_voiceRoom", "", CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_SYSTEMINFO, "Optional external voice relay room");
	// systeminfo
	...
	for(index = 4; index < MAX_MASTER_SERVERS; index++)
	...
	sv_lanForceRate = Cvar_Get ("sv_lanForceRate", "0", CVAR_ARCHIVE_ND );
