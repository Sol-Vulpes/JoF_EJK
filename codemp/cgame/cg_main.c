...
//	CG_LoadingString( "sounds" );
	...
	//cgs.media.blackIgnitionFlare = trap->R_RegisterShader( "gfx/effects/sabers/flare1black" );
...
//JAPRO - Clientside - Hitsounds End
	...
	//register tripmine model here
	cgs.media.tripMineModel = trap->R_RegisterModel("models/weapons2/laser_trap/laser_trap_w.glm");
...
*/
	...
	//if (cgs.serverMod != SVMOD_JAPLUS && cgs.serverMod != SVMOD_BASEJKA) {
		...
		//cgs.media.cosmetics.fire		= trap->FX_RegisterEffect("env/small_fire"); //art - memes
	//}
...
void CG_LoadMenus(const char *menuFile)
{
	...
	if ( !f )
	{
		...
		if ( !f )
		{
			trap->Print( S_COLOR_RED "default hud menu file not found: ui/jahud.txt, falling back to simple HUD" );
			trap->FS_Close( f );
			trap->Cvar_Set( "cg_hudFiles", "1" );
			trap->Cvar_Update( &cg_hudFiles );
			cgs.newHud = qtrue;
			return;
		}
	}
...
#if 0
	if (Q_strchrs(cgs.mapname, "ffa3")) {
		Com_Printf("^3patching bugged ffa3 shaders\n");
		if (trap->R_RegisterShader("models/map_objects/imperial/airpure_fixed")) {
			Com_Printf("^3remapping models/map_objects/imperial/airpure_fixed\n");
			trap->R_RemapShader("models/map_objects/imperial/airpure", "models/map_objects/imperial/airpure_fixed", NULL);
		}
		if (trap->R_RegisterShader("models/map_objects/ships/cart_fixed")) {
			Com_Printf("^3remapping models/mapobjects/ships/cart\n");
			trap->R_RemapShader("models/map_objects/ships/cart", "models/map_objects/ships/cart_fixed", NULL);
		}
	}
#endif
}
