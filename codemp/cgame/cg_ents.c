...
void CG_GrappleEndpoint( centity_t *ent, vec3_t endPos ) {
	...
	VectorCopy( endPos, model.lightingOrigin );
	VectorCopy( endPos, model.origin );
...
void CG_GrappleStartpoint( vec3_t startPos, vec3_t color ) {
	...
	for (i = 0; i < 3; i++) model.shaderRGBA[i] = color[i] * 255;
...
#if 0
//void CG_GrappleTrail_Original( centity_t *ent ) {
...
static void CG_GrappleTrail( vec3_t startPos, vec3_t endPos, vec3_t color ) {
	...
	//CG_GrappleEndpoint(ent, beam.oldorigin);
	//CG_GrappleStartpoint(ent, beam.origin);
	//should dynamic light re-implemented from below?
	...
	*/
...
#if 1 //ok unlagged ents take #2?
	qboolean unlagEnts = (qboolean)(cgs.serverMod == SVMOD_JAPRO && !ps->stats[STAT_RACEMODE] && !cg.demoPlayback && !cgs.localServer
									&& (cgs.jcinfo & (JAPRO_CINFO_UNLAGGEDPROJ|JAPRO_CINFO_UNLAGGEDHITSCAN)));
	unlagEnts = unlaggedEntsOverride.integer;// qtrue; //debug
	//loda - JAPRO unlagged ents - early transitioning - end
	if ( unlagEnts && cg.nextSnap ) {
		for ( num = 0 ; num < cg.nextSnap ->numEntities ; num++ ) {
			if (cg.nextSnap->entities[ num ].number != cg.nextSnap->ps.clientNum) { //bucky: checking this here since raven had changed this in JK2/JKA
				cent = &cg_entities[ cg.nextSnap->entities[ num ].number ];
				if ( cent->nextState.eType == ET_MISSILE || cent->nextState.eType == ET_GENERAL ) {
					//bucky: i don't know if we *really* want to do this on all ET_GENERALs but we'll try it for now
	...
	for ( num = 0 ; num < cg.snap->numEntities ; num++ ) {
		...
		if (cg.snap->entities[ num ].number != cg.snap->ps.clientNum)
		{
			...
			//unlagged - early transitioning
			if (unlagEnts && cg.nextSnap && (cent->nextState.eType == ET_MISSILE || cent->nextState.eType == ET_GENERAL))
				continue;
