...
static int CG_RadialMenuSelection( float x, float y ) {
	...
	return ( (int)( angle / ( (float)M_PI * 2.0f / 8.0f ) ) ) & 7;
}
...
void CG_RadialMenuDraw( void ) {
    static const vec4_t idleColor = { 0.08f, 0.08f, 0.08f, 0.82f };
    static const vec4_t activeColor = { 0.78f, 0.64f, 0.22f, 0.90f };
    static const vec4_t abortColor = { 0.45f, 0.10f, 0.10f, 0.88f };
    static const vec4_t abortActiveColor = { 0.82f, 0.20f, 0.20f, 0.94f };
    static const vec4_t deadColor = { 0.16f, 0.16f, 0.16f, 0.90f };
    static const vec4_t textColor = { 1.0f, 1.0f, 1.0f, 1.0f };
    float offsetX = 0.0f;
    float offsetY = 0.0f;
    const float boxW = Q_max( 24.0f, cg_radialBoxWidth.value );
    const float boxH = Q_max( 12.0f, cg_radialBoxHeight.value );
    const float textScale = Q_max( 0.12f, cg_radialTextScale.value );
    const int textLimit = (int)Q_max( 8.0f, boxW - 14.0f );
    const float ringRadius = Q_max( boxW * 0.52f, cg_radialRadius.value * 0.62f );
    const float deadzone = Q_max( 7.0f, cg_radialDeadzone.value * 0.5f );
    float centerX;
    float centerY;
    int i;
    ...
    for ( i = 0; i < 8; i++ ) {
        vmCvar_t *bind = CG_RadialBindCvar( i );
        const float angle = ( (float)M_PI * 2.0f / 8.0f ) * i - (float)M_PI * 0.5f;
        const float px = centerX + cosf( angle ) * ringRadius;
        const float py = centerY + sinf( angle ) * ringRadius;
        const vec4_t *color = CG_RadialMenuIsAbortSlot( i ) ? ( ( i == cgs.radialMenuSelection ) ? &abortActiveColor : &abortColor ) : ( ( i == cgs.radialMenuSelection ) ? &activeColor : &idleColor );
        char label[64];

        if ( CG_RadialMenuIsAbortSlot( i ) ) {
            Q_strncpyz( label, "Abort", sizeof( label ) );
        }
        else {
            Q_strncpyz( label, bind ? bind->string : "", sizeof( label ) );
            if ( !label[0] ) {
                Q_strncpyz( label, "<empty>", sizeof( label ) );
            }
        }

        CG_FillRect( px - boxW * 0.5f, py - boxH * 0.5f, boxW, boxH, *color );
        CG_Text_Paint( px - ( boxW * 0.5f ) + 5.0f, py + 10.0f, textScale, textColor, label, 0.0f, textLimit, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL2 );
    }

    CG_FillRect( centerX - deadzone, centerY - deadzone, deadzone * 2.0f, deadzone * 2.0f, deadColor );
}
...
#if TRAP_DIAGRAM_HELPER
static void CG_TrapDiagramHelper(centity_t *cent);
#endif
...
float PM_GroundDistance2(void)
{
	...
	down[2] -= 4096;
	CG_Trace(&tr, ps->origin, NULL, NULL, down, ps->clientNum, MASK_SOLID);
	VectorSubtract(ps->origin, tr.endpos, down);

	return VectorLength(down) - ps->viewheight;
...
void CG_DrawHUD(centity_t	*cent)
{
	...
	if (cgs.newHud) {
		switch (cg_hudFiles.integer)
		{
			...
			case 1: speedometerXPos -= 56;
				if (cg.snap->ps.stats[STAT_ARMOR] > 999) speedometerXPos += 4; break;
...
#if TRAP_DIAGRAM_HELPER
	if (cg_trapDiagram.integer)
		CG_TrapDiagramHelper(cent);
#endif
...
==================
*/
#define	FPS_FRAMES	90 //increased from 16 for accuracy
...
static void CG_DrawInventory(int y)
{
	...
	if (cgs.serverMod == SVMOD_JAPRO && cg.snap->ps.stats[STAT_RACEMODE] && cg.snap->ps.stats[STAT_MOVEMENTSTYLE] < MV_COOP_JKA)
		return;
...
static void CG_DrawLagometer( void ) {
	...
	if (!cg_lagometer.integer || (cgs.localServer && !cg_drawSnapshot.integer)) {
		CG_DrawDisconnect();
		return;
	}
...
static void CG_DrawCrosshair( vec3_t worldPoint, int chEntValid ) {
	...
	if ( cg_crosshairHealth.integer )
	{
	...
	else if (cg_drawCrosshair.integer == 10 || cg_drawCrosshair.integer == 11) {
		hcolor[0] = 1.0f;
		hcolor[1] = 1.0f;
		hcolor[2] = 1.0f;
		hcolor[3] = 3.0f;
		...
		if (cg_drawCrosshair.integer == 11) {
			const float screenXScale = (cgs.glconfig.vidWidth * (1.0 / SCREEN_WIDTH));
			const float screenYScale = (cgs.glconfig.vidHeight * (1.0 / SCREEN_HEIGHT));
			...
			x = SCREEN_WIDTH / 2;
			y = SCREEN_HEIGHT / 2;
			const float gap = 14/screenXScale;

			CG_DrawPic(x + 1, y + 1, w, h, hShader);
			CG_DrawPic(x - 1, y - 1, w, h, hShader);
			CG_DrawPic(x, y + 14, w, h, hShader);
			CG_DrawPic(x, y - 14, w, h, hShader);
			CG_DrawPic(x + 14, y, w, h, hShader);
			CG_DrawPic(x - 14, y, w, h, hShader);

		}
	}
	else if (cg.crosshairColor[0] > 0.0f && cg.crosshairColor[1] > 0.0f && cg.crosshairColor[2] > 0.0f)
	{
		hcolor[0] = cg.crosshairColor[0];
		hcolor[1] = cg.crosshairColor[1];
		hcolor[2] = cg.crosshairColor[2];
	...
	{
		ecolor[3] = 0.5f;
		ecolor[0] = ecolor[1] = ecolor[2] = (1.0f - ecolor[3]) * ( sin( cg.time * 0.001f ) * 0.08f + 0.35f ); // don't draw full color
		ecolor[3] = 1.0f;
		...
		trap->R_DrawStretchPic(x + cg.refdef.x + 0.5 * (SCREEN_WIDTH - w),
...
void CG_DrawEnhancedFlagStatus(void)
{
	...
	if (team == TEAM_RED)
	{
		...
		if (cgs.redFlagCarrier && cgs.redFlagCarrier->name[0] && cgs.blueflag == FLAG_TAKEN) {
			...
			armor = cgs.serverMod == SVMOD_JAPRO ? (cgs.redFlagCarrier->armor / 100) - 1 : cgs.redFlagCarrier->armor;
			CG_GetColorForHealth(cgs.redFlagCarrier->health, armor, hcolor);

			if (cgs.redFlagCarrier->armor)
				Com_sprintf(myFlagStatusHP, sizeof(myFlagStatusHP), "(%i/%i)", cgs.redFlagCarrier->health, armor);
			else
	...
	else
	{
		...
		if (cgs.blueFlagCarrier && cgs.blueFlagCarrier->name[0] && cgs.redflag == FLAG_TAKEN) {
			...
			armor = cgs.serverMod == SVMOD_JAPRO ? (cgs.blueFlagCarrier->armor / 100) - 1 : cgs.blueFlagCarrier->armor;
			CG_GetColorForHealth(cgs.blueFlagCarrier->health, armor, hcolor);

			if (cgs.blueFlagCarrier->armor)
				Com_sprintf(myFlagStatusHP, sizeof(myFlagStatusHP), "(%i/%i)", cgs.blueFlagCarrier->health, armor);
			else
...
static QINLINE void CG_ChatBox_DrawStrings(void)
{
	...
	{
		if (cg.pressingScoreBoard && cgs.numClients > 26)
			y = 10000;
		else
			y = 475;
	}
...
static void CG_TrapDiagramHelper(centity_t *cent) {
	...
	playerState_t *ps = &cg.predictedPlayerState;
	trace_t trace = { 0 };
	vec3_t pathStart = { 0 }, pathEnd = { 0 };

	if (!cg.snap || !ps || !cent)
		return;

	//if (ps->movementDir && ps->movementDir != 2 && ps->movementDir != 6)
	if (cg.currentSpeed > 0.0f && ps->movementDir != 2 && ps->movementDir != 6) //not moving left or right
		return;

	lastAttacker = CG_LastAttacker();

	if (cg_trapDiagramTarget.integer >= 0 && cg_trapDiagramTarget.integer < MAX_CLIENTS) { //temp cvar
		enemy = &cg_entities[cg_trapDiagramTarget.integer];
	}
	else if (ps->duelInProgress && ps->duelIndex >= 0 && ps->duelIndex < MAX_CLIENTS) { //get dueler
		enemy = &cg_entities[ps->duelIndex];
	}
	else if (lastAttacker >= 0 && lastAttacker < MAX_CLIENTS) {
		enemy = &cg_entities[lastAttacker];
	}
	/*else { //get closest player? idk..
		for (i = 0; i < MAX_CLIENTS; i++) {

		}
	}*/

	if (!enemy || !enemy->currentValid || enemy->currentState.eType != ET_PLAYER || (enemy->currentState.eFlags & EF_DEAD)) // not a valid target
		return;

	if (cent->currentState.groundEntityNum == ENTITYNUM_NONE)// || enemy->currentState.groundEntityNum == ENTITYNUM_NONE)
		return;

	/*VectorCopy(ps->origin, pathStart);
	pathStart[2] -= 4096;//8192 would probably better
	CG_Trace(&trace, ps->origin, NULL, NULL, pathStart, ps->clientNum, MASK_SOLID);*/
	VectorCopy(cent->lerpOrigin, pathStart);
	//pathStart[2] -= 4096;//8192 would probably better
	pathStart[2] -= 8192;
	CG_Trace(&trace, cent->lerpOrigin, NULL, NULL, pathStart, cent->currentState.clientNum, MASK_SOLID);
	VectorCopy(trace.endpos, pathStart);

	Com_Memset(&trace, 0, sizeof(trace_t));

	VectorCopy(enemy->lerpOrigin, pathEnd);
	//pathEnd[2] -= 4096;//8192 would probably better
	pathEnd[2] -= 8192;
	CG_Trace(&trace, enemy->lerpOrigin, NULL, NULL, pathEnd, enemy->currentState.number, MASK_SOLID);
	VectorCopy(trace.endpos, pathEnd);

	//Draw the line
	CG_TestLine(pathStart, pathEnd, (cg.frametime*2), -1, 3.2f);

	//addbezierArgStruct_t bezier = { 0 };
	//trap->FX_AddBezier()

	//TODO://trace both positions to floor level
	//offset target position based on enemy's look direction and offset it behind their back
	//(most difficult prolly): calculate curve trajectories from local player for both strafe directions that matches diagram below
	////we already have start point and end point, so calculate the furthest point in the curve for each sides of the figure 8, work from there to smooth it out?
	//color trajectory green if it's the current moveDir?
	/*
            xxxx           oooo
    ...
    -           O         X            +
      ...
      x           o     x            o
		...
		-You are O(uppercase). Your trapping cirle is marked by o's(lowercase).
		-Where ever your opponent is, he is X.
		-the close you get the smaller your trapping circle gets up until he is in your range, then you are at either spot R or spot L where you go for hits.
		-do not touch W or S for this footwork.
		-your aim is to get to the + mark as much as possible. the + is behind your opponent.
		-do not allow your opponent to the - mark. consider that as taking a - to your health. 4-5 losses at marks R, L, or - and you're dead.*/
}
#endif
...
static void CG_DrawShowPos(void)
{
	...
	if (!ps->m_iVehicleNum) {
		vel = VectorLength(ps->clientNum == cg.clientNum ? ps->velocity : cg_entities[ps->clientNum].currentState.pos.trDelta);
	}
	else {
		centity_t *vehCent = &cg_entities[ps->m_iVehicleNum];
		vel = VectorLength(ps->clientNum == cg.clientNum ? vehCent->playerState->velocity : vehCent->currentState.pos.trDelta);
	}


	if (cg.renderingThirdPerson) {
		Com_sprintf(showPosString, sizeof(showPosString),
			"pos:   %.2f   %.2f   %.2f\n"
			"ang:   %.2f   %.2f %.2f\n"
			"vel:     %.2f",
			ps->origin[0], ps->origin[1], ps->origin[2],
			ps->viewangles[PITCH], ps->viewangles[YAW], ps->viewangles[ROLL], vel);
	}
	else {
		Com_sprintf(showPosString, sizeof(showPosString),
			"pos:   %.2f   %.2f   %.2f\n"
			...
			"vel:     %.2f",
			ps->origin[0], ps->origin[1], ps->origin[2],
			cg.refdef.viewangles[PITCH], cg.refdef.viewangles[YAW], cg.refdef.viewangles[ROLL], vel);
	}

	CG_Text_Paint((float)SCREEN_WIDTH * 0.64f, 0, 1.125f / ((float)cgs.glconfig.vidHeight * (1.0f / SCREEN_HEIGHT)),
		colorWhite, showPosString, 0, 0, (ITEM_ALIGN_LEFT|ITEM_TEXTSTYLE_OUTLINESHADOWED), FONT_SMALL2);
}

static void CG_MovementKeys(centity_t *cent)
{
		usercmd_t cmd = { 0 };
		qboolean isSpectating = qfalse;
		...
		isSpectating = (qboolean)(cg.clientNum != cg.predictedPlayerState.clientNum || cg.demoPlayback || cg.predictedPlayerState.pm_type == PM_SPECTATOR || (cg.predictedPlayerState.pm_flags & PMF_FOLLOW));
		ps = &cg.snap->ps; //&cg.predictedPlayerState;
