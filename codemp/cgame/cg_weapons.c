...
void CG_Weapon_f( void ) {
	...
	if (!Q_stricmp(CG_Argv(1), "stun")) {
		...
		{
			if (!(ps->stats[STAT_WEAPONS] & (1<<WP_BRYAR_PISTOL)) && !(ps->stats[STAT_WEAPONS] & (1<<WP_BRYAR_OLD)))
			{ //can't use either pistol
				if ((ps->stats[STAT_WEAPONS] & (1<<WP_MELEE)))//switch to melee
					num = WP_MELEE;
				else if (ps->stats[STAT_WEAPONS] & (1<<WP_STUN_BATON)) //switch to stun baton if melee isnt available
					num = WP_STUN_BATON;
			}
			else if (ps->stats[STAT_WEAPONS] & (1 << WP_BRYAR_OLD))
			{//hack to make pistol button equip bryar or cycle between bryar and dl-44
				if (ps->weapon == WP_BRYAR_PISTOL || !(ps->stats[STAT_WEAPONS] & (1 << WP_BRYAR_PISTOL)))
					num = WP_BRYAR_OLD;
				else if (ps->weapon == WP_BRYAR_OLD)
					num = WP_BRYAR_PISTOL;
			}
		}*/
		//shitty weapon bucket system
		if (num == WP_BRYAR_PISTOL)
		{
