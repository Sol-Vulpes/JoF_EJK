...
qboolean CG_OtherTeamHasFlag(void) { //modified to return true if flag has been dropped
...
qboolean CG_YourTeamHasFlag(void) { //modified to return true if flag has been dropped
	if (cgs.gametype == GT_CTF || cgs.gametype == GT_CTY) {
		int team = cg.snap->ps.persistant[PERS_TEAM];
