...
#ifdef phoondemotech
	int secs, mins;
	secs = (cg.time - cgs.levelStartTime) / 1000;
	mins = secs / 60;
	secs %= 60;
#endif
...
#ifdef phoondemotech
		if (cg.demoPlayback) {
			if (cg_demoKillPrint.integer == 2 && attacker == cg.snap->ps.clientNum) {
				CG_Printf("[skipnotify]%s pivotfrag\n", cg.demoTimeStamp, attackerName);
			}
			else if (cg_demoKillPrint.integer && cg_demoKillPrint.integer != 2)
				CG_Printf("[skipnotify]%s %s fragged som1\n", cg.demoTimeStamp, Info_ValueForKey(attackerInfo, "n"));
			//CG_Printf("^7[^2i:02i^7] %s fragged som1\n", mins, secs, Info_ValueForKey(attackerInfo, "n"));
		}
#endif
	}
