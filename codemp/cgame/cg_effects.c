...
#define MAX_FPSSAVER_EFFECTS    (128)
static fpsSaverEffect_t fpsSaverEffects[MAX_FPSSAVER_EFFECTS] = { 0 };

#define FPSSAVER_EFFECTSPAM_FUZZ            (16.0f)
#define FPSSAVER_EFFECTSSPAM_DEBOUNCE_MS    (50)

// compares vectors with some wiggle room if they aren't EXACTLY the same
static qboolean FuzzyVectorCompare(const vec3_t vec1, const vec3_t vec2, float fuzz) {
	assert(fuzz >= 0.0f);
	if (fabs(vec1[0] - vec2[0]) > fuzz)
		return qfalse;
	if (fabs(vec1[1] - vec2[1]) > fuzz)
		return qfalse;
	if (fabs(vec1[2] - vec2[2]) > fuzz)
		return qfalse;
	return qtrue;
}

static qboolean ShouldPlayEffect(int id, vec3_t org) {
	// initialize times as negative numbers so that if an effect is played at exactly 0:00,
	// there will always be something "earlier"
	static qboolean initialized = qfalse;
	if (!initialized) {
		for (int i = 0; i < MAX_FPSSAVER_EFFECTS; i++)
			fpsSaverEffects[i].time = -FPSSAVER_EFFECTSSPAM_DEBOUNCE_MS;
		initialized = qtrue;
	}

	// see if this id+org already exists in the array
	// also, in case it doesn't, determine an old one for replacement
	fpsSaverEffect_t *found = NULL, *old = NULL;
	for (int i = 0; i < MAX_FPSSAVER_EFFECTS; i++) {
		if (!found && fpsSaverEffects[i].id == id && FuzzyVectorCompare(org, fpsSaverEffects[i].org, FPSSAVER_EFFECTSPAM_FUZZ))
			found = &fpsSaverEffects[i];
		if (!old && cg.time - fpsSaverEffects[i].time >= FPSSAVER_EFFECTSSPAM_DEBOUNCE_MS)
			old = &fpsSaverEffects[i];
		if (found && old)
			break;
	}

	if (!found) { // if this id+org isn't in the array, replace the old one
		found = old ? old : &fpsSaverEffects[0]; // if no old one, just use [0] (should never happen)
		found->id = id;
	}
	VectorCopy(org, found->org); // update the org

	if (cg.time - found->time < FPSSAVER_EFFECTSSPAM_DEBOUNCE_MS) // don't spam more than 20Hz
		return qfalse;
...
// for spammy effects such as saber sparks, otherwise just call trap->FX_PlayEffectID directly.
//void CG_FX_PlayEffectID_FpsSaver(int id, vec3_t org, vec3_t fwd, int vol, int rad)
void CG_FX_PlayEffectID(int id, vec3_t org, vec3_t fwd, int vol, int rad, qboolean FPSSaver)
{
	if (FPSSaver && !ShouldPlayEffect(id, org))
		return;
	trap->FX_PlayEffectID(id, org, fwd, vol, rad, qfalse);
}

/*// for normal effects with no fps saving
void trap_FX_PlayEffectID( int id, vec3_t org, vec3_t fwd, int vol, int rad )
{
	syscall( CG_FX_PLAY_EFFECT_ID, id, org, fwd, vol, rad );
}

// for spammy effects such as saber sparks
void trap_FX_PlayEffectID_FpsSaver(int id, vec3_t org, vec3_t fwd, int vol, int rad)
{
	if (!ShouldPlayEffect(id, org))
		return;
	syscall(CG_FX_PLAY_EFFECT_ID, id, org, fwd, vol, rad);
}*/
#endif
