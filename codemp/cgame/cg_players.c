...
void CG_NewClientInfo( int clientNum, qboolean entitiesInitialized ) {
	...
	if (cgs.gametype >= GT_TEAM	&& !cgs.jediVmerc && cgs.gametype != GT_SIEGE )
	{ //We won't force colors for siege.
		//Team skins are transient; keep the archived model cvar unchanged so non-team games restore the player's chosen skin.
...
static void CG_SetLerpFrameAnimation( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float animSpeedMult, qboolean torsoOnly, qboolean flipState) {
	...
	if (cent->ghoul2)
	{
		...
		int firstFrame;
		int lastFrame;
#if 1//def TECH//#if 0 //disabled for now //Milamber: leaving this for now, it's pre-existing code just enabled.
		float unused;
#endif
...
#if 1
#define SFX_SABER_ST 1.0f //1.0f seems to make more consistent trail fragments?
			fx.mVerts[0].ST[0] = 0.0f;
			fx.mVerts[0].ST[1] = SFX_SABER_ST;
			fx.mVerts[0].destST[0] = SFX_SABER_ST;
			fx.mVerts[0].destST[1] = SFX_SABER_ST;
			...
			fx.mVerts[1].ST[0] = 0.0f;
			fx.mVerts[1].ST[1] = 0.0f;
			fx.mVerts[1].destST[0] = SFX_SABER_ST;
			fx.mVerts[1].destST[1] = 0.0f;
			...
			fx.mVerts[2].ST[0] = SFX_SABER_ST;
			fx.mVerts[2].ST[1] = 0.0f;
			fx.mVerts[2].destST[0] = SFX_SABER_ST;
			fx.mVerts[2].destST[1] = 0.0f;
			...
			fx.mVerts[3].ST[0] = SFX_SABER_ST;
			fx.mVerts[3].ST[1] = SFX_SABER_ST;
			fx.mVerts[3].destST[0] = SFX_SABER_ST;
			fx.mVerts[3].destST[1] = SFX_SABER_ST;
#undef SFX_SABER_ST
#else
			fx.mVerts[0].ST[0] = 0.0f;
...
//JAPRO - Clientside - Draw Player Collision Hitbox - Start
	if (cg_drawHitBox.integer) {
			...
			cent->currentState.trickedentindex4, cg.snap->ps.clientNum))
		{
			...
			VectorAdd( cent->lerpOrigin, bmins, absmin );
			VectorAdd( cent->lerpOrigin, bmaxs, absmax );
			CG_CubeOutline( absmin, absmax, 1, -1, 0.25 );
...
// minimal_add:
		...
		&& cent->currentState.weapon == WP_SABER && !BG_InSlopeAnim(cent->currentState.legsAnim))
	{
		...
		/*if (cent->currentState.torsoAnim == BOTH_RUN2)
			cent->currentState.torsoAnim = BOTH_RUN1;
