...
#ifndef _CGAME
				if (i == item->cursorPos) {
					//if ((int)item->special != FEEDER_Q3HEADS || (int)trap->Cvar_VariableValue("ui_selectedModelIndex") > -1)
...
qboolean ItemParse_cvarStrList( itemDef_t *item, int handle ) {
	...
	if (!Q_stricmp(token.string, "feeder") && item->special == FEEDER_SCREEN_RESOLUTIONS) {
#ifdef UI_BUILD
		for (; multiPtr->count < uiInfo.uiDC.resolutionCount; multiPtr->count++) { //fill ui_vid_modes here?
			multiPtr->cvarList[multiPtr->count] = uiInfo.uiDC.resolutions[multiPtr->count];
			multiPtr->cvarStr[multiPtr->count] = uiInfo.uiDC.resolutions[multiPtr->count];
		}
#endif
		return qtrue;
	}
