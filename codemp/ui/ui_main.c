...
#else
static const char* masterServerNames[] = {
	"Raven Software",
	"JKHub"
};
static const size_t masterServerNameCount = ARRAY_LEN(masterServerNames);

static const char *GetMasterServerNameString(const char *sv_master) {
	if (Q_stristr(sv_master, "ravensoft"))
		return masterServerNames[0];
	if (Q_stristr(sv_master, "jkhub"))
		return masterServerNames[1];
	return sv_master;
}

static const int numNetSources = 8;	// now hard-entered in StringEd file //ok but where???
static const char *GetNetSourceString(int iSource)
{
	static char result[256] = {0};

	if ( iSource >= UIAS_GLOBAL1 && iSource <= UIAS_GLOBAL5 ) {
		char *sv_master = UI_Cvar_VariableString(va("sv_master%i", iSource));
		if (VALIDSTRING(sv_master) && strlen(sv_master)) {
			Q_strncpyz(result, GetMasterServerNameString(sv_master), sizeof(result));
			return result;
		}
	}
...
void UI_UpdateCurrentServerInfo(void) { //parses server info to contextually hide items/menus depending on server settings/mod
	...
	value = Info_ValueForKey(info, "sv_pure");
	if (VALIDSTRING(value) && atoi(value)) {
		trap->Cvar_Set("ui_sv_pure", "1"); //triggers cvar update func next frame
	}
...
void _UI_DrawSides(float x, float y, float w, float h, float size) {
	...
	//size *= uiInfo.uiDC.widthRatioCoef;
...
void Text_PaintWithCursor(float x, float y, float scale, vec4_t color, const char *text, int cursorPos, char cursor, int limit, int style, int iMenuFont)
{
	...
	//
	{
		...
		int iCopyCount = limit > 0 ? Q_min( (int)strlen( text ), limit ) : (int)strlen( text );
			...
			{
				int iFontIndex = MenuFontToHandle( iMenuFont );
				int iNextXpos  = trap->ext.R_Font_StrLenPixels(sTemp, iFontIndex, scale );
...
static void Text_Paint_Limit(float *maxX, float x, float y, float scale, vec4_t color, const char* text, float adjust, int limit, int iMenuFont)
{
	...
	//float fMax = *maxX;
	int iPixelLen = trap->ext.R_Font_StrLenPixels(text, iFontIndex, scale);
	if (x + iPixelLen > *maxX)
	{
		...
		while (*psText && (x + trap->ext.R_Font_StrLenPixels(sTemp, iFontIndex, scale)<=*maxX)
...
=================
*/
uiQ3ModelBuild_t uiQ3ModelBuild;
...
*/
	...
	if (menu)
	{
		...
		//fix saber only setting
		i = trap->Cvar_VariableValue("g_weaponDisable");
		if (i == 1) { //this is set by the menu when the "saber only" setting is turned on
			int weaponDisable = 0;
			for (i = 0; i < WP_NUM_WEAPONS; i++) {
				if (i != WP_SABER)
					weaponDisable |= (1 << i);
			}
			trap->Cvar_SetValue("g_weaponDisable", weaponDisable);
		}
#endif
		...
		//if (ui_forcePowerDisable.integer)
		//if (atoi(Info_ValueForKey(info, "g_forcePowerDisable")))
#if 1 //idk this worked fine in vanilla apparently
		disabledForce = atoi(Info_ValueForKey(info, "g_forcePowerDisable"));
#else
		if (ui_forcePowerDisable.integer > 0) { //fix NF
			if (ui_forcePowerDisable.integer == 1) //^^you can't tell me what to do
			{ //disable all force powers (should this allow force jump like most NF servers?)
				for (i = 0; i < NUM_FORCE_POWERS; i++) {
					if (i == FP_LEVITATION || i == FP_SABER_OFFENSE || i == FP_SABER_DEFENSE)
						continue;
					disabledForce |= (1 << i);
				}
				//sanity check player's forcepower setting
				/*i = trap->Cvar_VariableValue("forcepowers");
				if (!i || !(i & FP_SABER_OFFENSE) || !(i & FP_SABER_DEFENSE)) {
					UI_ForceConfigHandle()
					trap->Cvar_SetValue("forcepowers", (float)(FP_SABER_OFFENSE|FP_SABER_DEFENSE));
					trap->Cmd_ExecuteText(EXEC_APPEND, "cmd forcechanged\n");
					UI_UpdateForcePowers();

				}*/
			}
			else { //custom forcePowerDisable setting - TODO: add menu which allows player allow/disallow each force power
				disabledForce = ui_forcePowerDisable.integer;
			}
			trap->Cvar_SetValue("g_forcePowerDisable", disabledForce);
		}
#endif
		allForceDisabled = UI_AllForceDisabled(disabledForce);
...
const char* detectedResolutions[MAX_RESOLUTIONS];
...
static int UI_FindBuiltinResolution(int mode)
{
	int i;

	if (!uiInfo.uiDC.resolutionsDetected)
		return mode;
...
static int UI_FindDetectedResolution(int mode)
{
	int i;
...
static void UI_GetResolutions(void)
{
	...
	if (*resbuf)
	{
		...
		if (i > 0)
		{
			uiInfo.uiDC.resolutions = detectedResolutions;
			uiInfo.uiDC.resolutionsDetected = qtrue;
			uiInfo.uiDC.resolutionCount = i;
		}
	}
	else {
		uiInfo.uiDC.resolutionsDetected = qfalse;
		uiInfo.uiDC.resolutions = builtinResolutions;
		uiInfo.uiDC.resolutionCount = ARRAY_LEN(builtinResolutions);
	}
}
...
=================
*/
static void UI_GetAspectRatios(void)
{
	int i, r;

	// build ratio list from resolutions
	for (r = 0; uiInfo.uiDC.resolutions[r]; r++)
	{
		int w, h;
		char *x;
		...
		x = strchr(uiInfo.uiDC.resolutions[r], 'x') + 1;
		Q_strncpyz(str, uiInfo.uiDC.resolutions[r], x - uiInfo.uiDC.resolutions[r]);
		w = atoi(str);
		h = atoi(x);
...
static void UI_UpdateCharacterCvars( void )
{
	...
	Com_sprintf( skin, sizeof( skin ), "%s/%s|%s|%s", model, head, torso, legs );
...
void UI_GetCharacterCvars( void )
{
	...
	else
	{
		...
		if (/*ui_selectedModelIndex.integer > -1 && */ui_gametype.integer < GT_TEAM) {
			...
			UI_SetTeamColorFromModel(modelWithSkin);
		}
#endif
...
static void UI_RunMenuScript(char **args)
{
	...
	if (String_Parse(args, &name))
	{
		if (Q_stricmp(name, "StartServer") == 0)
		{
			...
			trap->Cvar_Update( &ui_netSource );
			if (ui_netSource.integer == UIAS_LOCAL/* || !uiInfo.serverStatus.numDisplayServers /*fuck off razor*/) {
				UI_StartServerRefresh(qtrue);
			}
...
#if 0 //works fine with OJK jampgame
...
#if 0 //probably not necessary now that the selectedmodel index updates with the model cvar
		trap->Cvar_Set("ui_selectedModelIndex", "-1");
		trap->Cvar_Update(&ui_selectedModelIndex);
#endif
...
static int UI_FeederCount(float feederID)
{
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_SCREEN_RESOLUTIONS:
			return uiInfo.uiDC.resolutionCount;
	}
...
static const char *UI_FeederItemText(float feederID, int index, int column,
	...
	*handle1 = *handle2 = *handle3 = -1;
	...
	else if (feederID == FEEDER_SCREEN_RESOLUTIONS) {
		if (index >= 0 && index < uiInfo.uiDC.resolutionCount)
		{
			return uiInfo.uiDC.resolutions[index];
		}
	}
...
static qhandle_t UI_FeederItemImage(float feederID, int index) {
	...
	else if (feederID == FEEDER_Q3HEADS)
	{
		...
		{ //we want it to load them as it draws them, like the TA feeder
			...
			int selModel = ui_selectedModelIndex.integer;
					...
					if (menu) {
						...
						for (i = 0; i < menu->itemCount; i++)
							if (menu->items[i]->special == FEEDER_Q3HEADS) {
								list = menu->items[i]->typeData.listbox;
								if (list) {
...
qboolean UI_FeederSelection(float feederFloat, int index, itemDef_t *item)
{
	...
	{
		trap->Cvar_SetValue("ui_r_mode", index);
	}
	else if (  feederID == FEEDER_SIEGE_BASE_CLASS )
	{
		int team,baseClass;
...
void UI_BuildQ3Model_List_ProcessDir(char* dirptr, char* filelist, int numfiles)
{
	...
	char* fileend = filelist + PROFILE_SKIN_SIZE;
	char skinname[64];
	int filelen;
	int j, k, p, s;
	...
	{
		const void* terminator = memchr(fileptr, '\0', (size_t)(fileend - fileptr));
		if (!terminator)
		{
			break;
		}
		filelen = (int)((const char*)terminator - fileptr);
		if (filelen <= 0)
		{
			continue;
		}
...
void UI_BuildQ3Model_List(char* dirptr, char* filelist, int bufsize)
{
	...
	// iterate directory of all player models
	numdirs = trap->FS_GetFileList("models/players", "/", dirptr, PROFILE_SKIN_SIZE);
	for (i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_Q3PLAYERMODELS; i++, dirptr += dirlen + 1)
	{
		...
		const int numfiles = trap->FS_GetFileList(va("models/players/%s", dirptr), "skin", filelist, bufsize);
		if (numfiles > 0)
			UI_BuildQ3Model_List_ProcessDir(dirptr, filelist, numfiles);
	}
}


void UI_BuildQ3Model_List_Process()
{
	const int maxDirsPerFrame = 1;
	int processed = 0;

	if (!uiQ3ModelBuild.inProgress)
		return;

	if (uiQ3ModelBuild.dirCount < 0)
	{
		int outLen = 0;
		const int st = trap->FS_AsyncStatus(uiQ3ModelBuild.dirJob, &outLen);
		if (st == UI_ASYNC_DONE)
		{
			const int count = trap->FS_AsyncRead(uiQ3ModelBuild.dirJob, uiQ3ModelBuild.dirList,
			                               sizeof(uiQ3ModelBuild.dirList));
			if (count <= 0)
			{
				trap->FS_AsyncFree(uiQ3ModelBuild.dirJob);
				uiQ3ModelBuild.inProgress = qfalse;
			...
			uiQ3ModelBuild.dirCount = count;
			uiQ3ModelBuild.dirPtr = uiQ3ModelBuild.dirList;
			uiQ3ModelBuild.dirIndex = 0;
		}
		else if (st == UI_ASYNC_ERROR || st == UI_ASYNC_CANCELLED || st == UI_ASYNC_INVALID)
		{
			trap->FS_AsyncFree(uiQ3ModelBuild.dirJob);
			uiQ3ModelBuild.inProgress = qfalse;
		}
	...
	while (processed < maxDirsPerFrame && uiQ3ModelBuild.dirIndex < uiQ3ModelBuild.dirCount)
	{
		if (uiQ3ModelBuild.fileJob != 0)
		{
			int outLen = 0;
			const int st = trap->FS_AsyncStatus(uiQ3ModelBuild.fileJob, &outLen);
			if (st == UI_ASYNC_DONE)
			{
				int count = trap->FS_AsyncRead(uiQ3ModelBuild.fileJob, uiQ3ModelBuild.fileList,
				                               sizeof(uiQ3ModelBuild.fileList));
				if (count > 0)
				{
					UI_BuildQ3Model_List_ProcessDir(uiQ3ModelBuild.dirName, uiQ3ModelBuild.fileList, count);
				}
				trap->FS_AsyncFree(uiQ3ModelBuild.fileJob);
				uiQ3ModelBuild.fileJob = 0;
				processed++;
			}
			else if (st == UI_ASYNC_ERROR || st == UI_ASYNC_CANCELLED || st == UI_ASYNC_INVALID)
			{
		...
		{
			const char* listEnd = uiQ3ModelBuild.dirList + sizeof(uiQ3ModelBuild.dirList);
			const void* terminator = memchr(uiQ3ModelBuild.dirPtr, '\0', (size_t)(listEnd - uiQ3ModelBuild.dirPtr));
			if (!terminator)
			{
				trap->FS_AsyncFree(uiQ3ModelBuild.dirJob);
				uiQ3ModelBuild.inProgress = qfalse;
			...
			int dirlen = (int)((const char*)terminator - uiQ3ModelBuild.dirPtr);
			const char* dirptr = uiQ3ModelBuild.dirPtr;
			qboolean baseSkin = qfalse;
			int k;

			uiQ3ModelBuild.dirPtr += dirlen + 1;
			uiQ3ModelBuild.dirIndex++;
			...
			Q_strncpyz(uiQ3ModelBuild.dirName, dirptr, sizeof(uiQ3ModelBuild.dirName));
			if (uiQ3ModelBuild.dirName[dirlen] == '/')
			{
				uiQ3ModelBuild.dirName[dirlen] = '\0';
			}

			if (!Q_stricmp(uiQ3ModelBuild.dirName, ".") || !Q_stricmp(uiQ3ModelBuild.dirName, ".."))
			{
				continue;
			}
			...
			if (ui_sv_pure.integer)
			{
				for (k = 0; k <= BASE_MODEL_COUNT; k++)
				{
					if (!Q_stricmp(uiQ3ModelBuild.dirName, baseModelList[k]))
					{
						baseSkin = qtrue;
						break;
					}
			...
			uiQ3ModelBuild.fileJob = trap->FS_GetFileListAsync(va("models/players/%s", uiQ3ModelBuild.dirName), "skin",
			                                                   sizeof(uiQ3ModelBuild.fileList));
		}
	}

	if (uiQ3ModelBuild.dirIndex >= uiQ3ModelBuild.dirCount && uiQ3ModelBuild.fileJob == 0)
	{
		trap->FS_AsyncFree(uiQ3ModelBuild.dirJob);
		uiQ3ModelBuild.inProgress = qfalse;
	}
...
static void UI_StartServerRefresh(qboolean full)
{
	...
	if( ui_netSource.integer >= UIAS_GLOBALS && ui_netSource.integer <= UIAS_GLOBAL5 ) {
		...
		if (strlen(ptr)) {
			...
			if (strlen(UI_Cvar_VariableString("com_protocol")) && strlen(UI_Cvar_VariableString("com_legacyprotocol"))) { //ETJK
				...
				trap->Cmd_ExecuteText( EXEC_NOW, va("globalservers %d %d full empty\n", ui_netSource.integer, (int)trap->Cvar_VariableValue("protocol")));
			}
		}
	}
#else
	if( ui_netSource.integer >= UIAS_GLOBALS && ui_netSource.integer <= UIAS_GLOBAL5 ) {
		...
		if (strlen(ptr)) {
			...
			char *com_protocol = UI_Cvar_VariableString("com_protocol"), *com_legacyprotocol = UI_Cvar_VariableString("com_legacyprotocol");
			if (strlen(com_protocol) && strlen(com_legacyprotocol)) { //ETJK
				trap->Cmd_ExecuteText(EXEC_NOW,
					va("globalservers %d %i full empty\n",
						ui_netSource.integer, (int)atoi(com_protocol)));
				trap->Cmd_ExecuteText(EXEC_NOW,
					va("globalservers %d %d full empty\n",
						ui_netSource.integer, (int)atoi(com_legacyprotocol)));
			}
			else {
				trap->Cmd_ExecuteText( EXEC_NOW, va("globalservers %d %d full empty\n", ui_netSource.integer, (int)trap->Cvar_VariableValue("protocol")));
			}
