...
qboolean PM_AdjustAngleForWallRunUp( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//wall-running up
		...
		{//hmm, some room, see if there's a floor right here
			...
			//Com_Printf("^6GAME: trace.fraction %f trace2.allsolid %s trace2.startsolid %s trace2.fraction %f trace2.plane.normal[2] %f\n", trace.fraction, trace2.allsolid ? "yes" : "no", trace2.startsolid ? "yes" : "no", trace2.fraction, trace2.plane.normal[2]);
			if ( !trace2.allsolid
				&& !trace2.startsolid
...
//JAPRO - Serverside + Clientside - Re add flipkick sidekick - End
				...
				//Com_Memset(&trace, 0, sizeof(trace)); //to shut the compiler up
...
static void PM_NoclipMove( void ) {
	...
	// Client-side fake noclip: when turbo-boosting (attack held), fly exactly along
	// the aim direction. PM_Accelerate uses the Quake momentum model - it only adds
	// speed *along* wishdir and never cancels perpendicular (sideways) velocity, so
	// when you're already moving fast horizontally and then aim up, currentspeed
	// stays high, addspeed collapses to ~0, and barely any upward speed is added -
	// the stale horizontal momentum just coasts on friction. That reads as "fast
	// horizontal, slow up", and no amount of acceleration fixes it because addspeed
	// (not accel) is the cap. So for turbo we set the velocity directly to the full
	// wish vector: instant, exact aim-tracking, capped at the turbo speed. Normal
	// (non-turbo) noclip keeps the smooth momentum-based feel.
	if ( pm->fakeNoclip && (pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) && wishspeed > 0.0f ) {
...
if (pm->ps->duelInProgress)
	...
	if (pm->ps->weapon == WP_SABER)
	{
		// Primary fire becomes the tactical-link gesture and alt fire clears all
		// links while viewing through binoculars with a fully holstered saber.
		// The server handles both actions; clearing them here also prevents client
		// prediction from igniting or attacking with the saber.
...
//[JAPRO - Serverside +clientside - Physics - Unlock bow movement/turning- End]
	...
	if (cgs.serverMod == SVMOD_JAPLUS) {
		if (!(pm->cmd.buttons & BUTTON_GRAPPLE)) {
			if (pm->ps->eFlags & EF_GRAPPLE_SWING) {
				//Com_Printf("aha yuh handle me\n");
				if (pm->cmd.buttons & BUTTON_USE)
					pm->ps->eFlags &= ~EF_GRAPPLE_SWING;
			}
			if (pm->ps->eFlags2 & EF2_GRAPPLE_OUT) {
				if (pm->cmd.buttons & BUTTON_USE)
					pm->ps->eFlags2 &= ~EF2_GRAPPLE_OUT;
				else if (!(pm->cmd.buttons & BUTTON_WALKING) && pm->cmd.upmove <= 0 &&
					pm->ps->groundEntityNum != ENTITYNUM_NONE) {//Prolly need to handle this properly in BG_AdjustClientSpeed...
					//pm->cmd.buttons |= BUTTON_WALKING;	//stiffenedUp = qtrue;
					pm->cmd.forwardmove = 0;
					pm->cmd.rightmove = 0;
				}
			}
			if ((pm->ps->pm_flags & PMF_GRAPPLE) && (pm->cmd.buttons & BUTTON_USE))
				pm->ps->pm_flags &= ~PMF_GRAPPLE;
		}
	}
#elif 0
	if (cgs.serverMod == SVMOD_JAPLUS && !(pm->cmd.buttons & BUTTON_GRAPPLE) && !(pm->cmd.buttons & BUTTON_USE))
	{
		if (pm->ps->eFlags & EF_GRAPPLE_SWING) {
			//pm->ps->pm_type = PM_FLOAT;
			//pm->ps->forceHandExtend = HANDEXTEND_NONE;
		}
		if (pm->ps->eFlags2 & EF2_GRAPPLE_OUT) {
			if (pm->cmd.upmove <= 0 && pm->ps->groundEntityNum != ENTITYNUM_NONE && !(pm->cmd.buttons & BUTTON_WALKING))
			{//Prolly need to handle this properly in BG_AdjustClientSpeed...
				//pm->cmd.buttons |= BUTTON_WALKING;	//stiffenedUp = qtrue;
				//pm->cmd.forwardmove = 0;
				//pm->cmd.rightmove = 0;
