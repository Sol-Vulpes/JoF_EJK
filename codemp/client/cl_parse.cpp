...
void CL_ParseServerMessage( msg_t *msg ) {
	...
	// get the reliable sequence acknowledge number
	int oldReliableAcknowledge = clc.reliableAcknowledge;
	clc.reliableAcknowledge = MSG_ReadLong( msg );
	//
	...
	if (clc.unique && clc.uniqueAssigned && cl_antiSpoof->integer)
	{
		...
		{
			clc.reliableAcknowledge = oldReliableAcknowledge;
			if (cl_showSpoofAttackMessages->integer)
			{
				Com_Printf("Likely spoof attempt - Disable this message with cl_showSpoofAttackMessages 0\n");
			}
			return;
		}
	}
