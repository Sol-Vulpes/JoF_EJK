		...
		if ( chatField.buffer[0] && cls.state == CA_ACTIVE ) {
			...
			if (cls.JKVersion != VERSION_1_01_CONSOLE) {
				...
				if ( chat_playerNum != -1 ) //pms are still broken? maybe they handle them like pc
					...
					Com_sprintf(buffer, sizeof(buffer), "say %s", coloredString);
				}
			}
#else
			CL_RandomizeColors(chatField.buffer, coloredString);
...
void Key_Rebind_f(void) {
	...
	if (!kg.keys[k1].binding || !kg.keys[k1].binding[0])
	{
		Com_Printf("\"%s\" is not bound\n", Key_KeynumToString(k1));
		return;
	}

	bind = kg.keys[k1].binding;
	key1 = Key_KeynumToString(k1);
	key2 = Key_KeynumToString(k2);
	if (kg.keys[k2].binding && kg.keys[k2].binding[0]) {
		Com_Printf("Swapped %s binding from %s to %s - ", kg.keys[k2].binding, key2, key1);
		Key_SetBinding(k1, kg.keys[k2].binding);
	}
	else {
		Key_SetBinding(k1, "");
	}
...
====================
*/
//extern void	Sys_SetSDLHint(const char *SDLHint, const char *value);
...
#if 0
	if (Cvar_VariableIntegerValue("in_mouse") != 2) {
		if (keyCatchers & KEYCATCH_UI)
			Sys_SetSDLHint("SDL_MOUSE_RELATIVE_MODE_WARP", "1");
		else
			Sys_SetSDLHint("SDL_MOUSE_RELATIVE_MODE_WARP", "0");
	}
#endif
}
