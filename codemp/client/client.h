...
typedef struct clientConnection_s {
	...
	int			reliableUnique;		// the last one the server has executed
...
typedef struct clientStatic_s {
	...
	//int				renderTime;
	//int				renderFrameTime; //redundant but whatever
#ifndef DEDICATED
	JKVersion_t		JKVersion;
#endif
...
extern cvar_t	*cl_showSpoofAttackMessages;
extern cvar_t	*cl_antiSpoof;
...
//#define PIPE_COMMAND_DEFAULT "ffmpeg -f avi -i - -threads 0 -preset ultrafast -y -pix_fmt yuv444p -crf 23 %o.mp4 2> ffmpeglog.txt" //current jaMME default setting
//#define PIPE_COMMAND_DEFAULT "ffmpeg -f avi -i - -threads 0 -preset veryfast -y -pix_fmt yuv444p -crf 16 %o.mp4 2> ffmpeglog.txt"
#define PIPE_COMMAND_DEFAULT "ffmpeg -f avi -i - -threads 0 -preset faster -y -pix_fmt yuv444p -crf 16 %o 2> ffmpeglog.txt"
//#define PIPE_COMMAND_DEFAULT "ffmpeg -f avi -i - -threads 0 -preset fast -y -pix_fmt yuv444p -crf 16 %o 2> ffmpeglog.txt" //probably no point to using fast/medium over faster at 180FPS...
//#define PIPE_COMMAND_DEFAULT cl_avipipecommand "ffmpeg -f avi -i - -vcodec h264_nvenc -threads 0 -preset lossless -profile high444p -level 4.2 -rc vbr_hq -y -pix_fmt yuv444p -crf 0 %o.mp4 2> ffmpeglog.txt"
//#define PIPE_COMMAND_DEFAULT cl_avipipecommand "ffmpeg -f avi -i - -f avi -vcodec h264_nvenc -threads 0 -preset lossless -profile high444p -rc vbr_hq -y -pix_fmt yuv444p -b:v 7500000 -crf 0 %o-piped.avi 2> ffmpeglog.txt"
