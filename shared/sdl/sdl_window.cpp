...
#endif
#if !defined(WIN32) && defined(SDL_VIDEO_DRIVER_WINDOWS) //what the fuck...
...
static SDL_Surface *cursorSurface = NULL;
...
{
#if WIN32 //keep us from crashing...
	int windowFlags = SDL_GetWindowFlags(screen);
	if ((windowFlags & SDL_WINDOW_VULKAN) && (!(windowFlags & SDL_WINDOW_BORDERLESS) || (windowFlags & SDL_WINDOW_FULLSCREEN))) {
		//Com_Printf("Avoiding GLimp_Minimize() crash...\n");
		return;
	}
#endif
...
void WIN_ShowCursor(qboolean show)
{
	if (ui_drawCursor && ui_drawCursor->integer) {
	...
	//cursorSurface = SDL_LoadBMP((1 < argc) ? argv[1] : "cursor.bmp");
	if (!cursorSurface) cursorSurface = SDL_LoadBMP("cursor1.bmp");
	if (!cursorSurface) {
		Com_Printf(S_COLOR_RED "failed to load cursor1.bmp\n");
		return;
	}

	if (!cursor) cursor = SDL_CreateColorCursor(cursorSurface, 0, 0);
	if (!cursor) {
		Com_Printf(S_COLOR_RED "failed to create color cursor\n");
		return;
	}
	...
	qboolean noborder)
{
	...
	//Uint32 flags = SDL_WINDOW_ALLOW_HIGHDPI; //If u even need anything at all
	Uint32 flags = 0;
	...
	else
	{
		...
		screen = SDL_CreateWindow(
			windowTitle, x, y, w, h, flags);
		/*if (!screen) {
			Com_Printf(S_COLOR_YELLOW "initial window creation failed? calling SDL_CreateWindow now?\n");
			screen = SDL_CreateWindow(
					windowTitle, x, y, w, h, flags);
		}
		else {
			...
			while (SDL_PollEvent(&e)) { //go ahead and process anything that's happened since we first started up
			}
		}*/
		if (screen == NULL)
		{
...
#if 0//def WIN32 //why the fuck is this still happening on jka but not on jk2??
...
window_t WIN_Init( const windowDesc_t *windowDesc, glconfig_t *glConfig )
{
	...
	Cvar_CheckRange(r_depthbits, 0, 32, qtrue); //max should be 24?
	r_colorbits			= Cvar_Get( "r_colorbits",			"0", CVAR_ROM );
	Cvar_CheckRange(r_colorbits, 0, 31, qtrue); //max should be 24?
