...
static void IN_ActivateMouse( void )
{
	...
	if (!reported) {
		//SDL_SpewWindowFlags(SDL_GetWindowFlags(SDL_window));
		Com_Printf("IN_ActivateMouse(), mouseAvailable %s, mouseActive %s, com_unfocused %i, com_minimized %i\n", mouseAvailable ? "yes" : "no", mouseActive ? "yes" : "no", com_unfocused->integer, com_minimized->integer);
		reported = qtrue;
	}
#endif
...
===============
*/
static cvar_t *ui_drawCursor = NULL;
...
static void IN_InitJoystick( void )
{
	...
	in_joystickThreshold = Cvar_Get( "joy_threshold", "0.30", CVAR_ARCHIVE_ND );

	stick = SDL_JoystickOpen( in_joystickNo->integer );

	in_activeJoystick = Cvar_Get("in_activeJoystick", "none", CVAR_ROM);
	...
	Cvar_Set( "in_activeJoystick", SDL_JoystickNameForIndex(in_joystickNo->integer) );
	...
	Com_Printf( "Threshold: %f\n", in_joystickThreshold->value );

	SDL_JoystickEventState(SDL_QUERY);
}

/*void Sys_SetSDLHint(const char *SDLHint, const char *value)
{
	if (!SDL_SetHint(SDLHint, value))
		Com_Printf(S_COLOR_RED "Sys_SetSDLHint: Failed to set hint %s with value %s!\n", SDLHint, value);
}*/
...
#if 0
	qboolean mouseGrabOk = qtrue;
	SDL_Event e;
	int oldWindowFlags = SDL_GetWindowFlags(SDL_window);
	while (SDL_PollEvent(&e)) { //just discard any events we've got queued up to this point?
		switch (e.type)
		{
			case SDL_WINDOWEVENT:
				switch (e.window.event)
				{
					...
					case SDL_WINDOWEVENT_ENTER:
						Com_Printf("ignoring some inheireted bullshit from startup %i\n", e.window.event);
						break;
					case SDL_WINDOWEVENT_LEAVE:
						Com_Printf("SDL_WINDOWEVENT_LEAVE ");
					default:
						Com_Printf("detected actual window focus loss? %i\n", e.window.event);
				...
				break;
			default:
				Com_Printf("unexpected SDL event queued at startup %i\n", e.type);
				break;
		}
	}
	/*SDL_Window *mouseWindow = NULL;
	mouseWindow = SDL_GetMouseFocus();
	if (mouseWindow) {
		Com_Printf("got mouseWindow?\n");
	}*/
	int newWindowFlags = SDL_GetWindowFlags(SDL_window);
	Com_Printf("oldWindowFlags %i newWindowFlags %i\n", oldWindowFlags, newWindowFlags);

	if (mouseGrabOk)
		IN_DeactivateMouse();
#endif

	//IN_DeactivateMouse( ); //INPUTFIX REMOVES THIS
	...
	while( SDL_PollEvent( &e ) )
	{
		switch( e.type )
		{
			case SDL_KEYDOWN:
				...
				else if ( kg.keys[A_CTRL].down && !kg.keys[A_ALT].down && !kg.keys[A_ALT2].down && key >= A_CAP_A && key <= A_CAP_Z )
			...
			case SDL_MOUSEMOTION:
				...
				{
					if (e.motion.timestamp > mouseMotionTime) {
						if (e.motion.x != -1)
							SDL_MouseCursorX = e.motion.x;
						if (e.motion.y != -1)
							SDL_MouseCursorY = e.motion.y;

						mouseMotionTime = e.motion.timestamp;
						if (e.motion.x == -1 || e.motion.y == -1)
							break;

						//mx = e.motion.x;
						//my = e.motion.y;
						if (in_useRelativeMouseCursor && !in_useRelativeMouseCursor->integer) {
							mx = e.motion.x;
							my = e.motion.y;
						}
						else
		...
		if (in_joystickUseAnalog->integer)
		{
			...
			for (i = 0; i < total; i++)
			{
				...
				if ( axis != stick_state.oldaaxes[i] )
				{
					...
					Sys_QueEvent( 0, SE_JOYSTICK_AXIS, i, axis, 0, NULL ); //NOWNOWNOW
...
void IN_Frame (void) {
	...
	{
		// Console is down
		//IN_DeactivateMouse(); //NOTE - THIS IS WHAT CAUSES THE MOUSE TO BE UNRESPONSIVE WHEN CONSOLE IS OPEN, AS WELL AS MAKING IT TWITCH TURN
	}
	else if ((keyCatchers & KEYCATCH_UI) && in_useRelativeMouseCursor && !in_useRelativeMouseCursor->integer)
	{ //menu is open
		IN_DeactivateMouse();
		if (!ui_drawCursor) {
			ui_drawCursor = Cvar_Get("ui_drawCursor", "1", CVAR_NONE, "");
		}
		if (flags & SDL_WINDOW_MOUSE_FOCUS) {
			//if ((flags & SDL_WINDOW_INPUT_FOCUS) && Cvar_VariableIntegerValue("ui_drawCursor"))
			if (ui_drawCursor->integer && (flags & SDL_WINDOW_INPUT_FOCUS))
				SDL_ShowCursor(0);
			else
				SDL_ShowCursor(1);
		}
	...
	else if (SDL_window && SDL_window != SDL_GetMouseFocus()) {
		IN_DeactivateMouse();
	}
	else {
		IN_ActivateMouse();
